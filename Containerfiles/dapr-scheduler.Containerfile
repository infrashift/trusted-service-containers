...
COPY src/dapr-scheduler/ /src/

ENV CGO_ENABLED=0 \
    GOOS=linux
# The smoke test is --help, not --version. cmd/scheduler/options/options.go builds its
...
LABEL org.opencontainers.image.source="${SOURCE_URL}" \
      ...
      org.opencontainers.image.title="Trusted Dapr Scheduler" \
      org.opencontainers.image.description="Dapr scheduler service for reminders and workflows, built from source on trusted UBI9 Micro" \
      ...
      io.openshift.tags="dapr,scheduler,workflows,ubi9,vetted"
...
COPY --from=builder /out/scheduler /usr/bin/scheduler
...
RUN set -euo pipefail; \
    ...
    printf 'dapr:x:1001:0:Trusted Dapr Scheduler service account:/home/dapr:/sbin/nologin\n' >> /etc/passwd; \
    mkdir -p /etc/pki/tls/certs /etc/ssl /home/dapr /var/lib/dapr/scheduler; \
    ...
    ln -sf /usr/bin/scheduler /scheduler; \
    chown -R 1001:0 /home/dapr /var/lib/dapr/scheduler; \
    chmod -R g=u /home/dapr /var/lib/dapr/scheduler
...
EXPOSE 50006 8080 9090

# The scheduler embeds etcd and IS stateful, unlike placement. Without a
# writable 1001:0-owned mount, actor reminders and workflow state are lost.
VOLUME /var/lib/dapr/scheduler
