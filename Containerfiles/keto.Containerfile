...
RUN set -euo pipefail; \
    GOARCH="${TARGETARCH}" go build -tags=json1 \
      -ldflags="-s -w \
        -X 'github.com/ory/keto/internal/driver/config.Version=${SOURCE_VERSION}' \
        -X 'github.com/ory/keto/internal/driver/config.Commit=${SOURCE_REF}' \
        -X 'github.com/ory/keto/internal/driver/config.Date=${SOURCE_DATE}'" \
      -o /out/keto . ; \
    /out/keto version
...
LABEL org.opencontainers.image.source="${SOURCE_URL}" \
      ...
      org.opencontainers.image.title="Trusted Ory Keto" \
      org.opencontainers.image.description="Ory Keto permission and relationship server, built from source on trusted UBI9 Micro" \
      ...
      io.openshift.tags="ory,keto,authorization,ubi9,vetted"
...
COPY --from=builder /out/keto /usr/bin/keto
...
RUN set -euo pipefail; \
    ...
    printf 'ory:x:1001:0:Trusted Ory Keto service account:/home/ory:/sbin/nologin\n' >> /etc/passwd; \
