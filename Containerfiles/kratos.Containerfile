...
RUN set -euo pipefail; \
    GOARCH="${TARGETARCH}" go build -tags=json1 \
      -ldflags="-s -w \
        -X 'github.com/ory/kratos/driver/config.Version=${SOURCE_VERSION}' \
        -X 'github.com/ory/kratos/driver/config.Commit=${SOURCE_REF}' \
        -X 'github.com/ory/kratos/driver/config.Date=${SOURCE_DATE}'" \
      -o /out/kratos . ; \
    /out/kratos version
...
LABEL org.opencontainers.image.source="${SOURCE_URL}" \
      ...
      org.opencontainers.image.title="Trusted Ory Kratos" \
      org.opencontainers.image.description="Ory Kratos identity and user management, built from source on trusted UBI9 Micro" \
      ...
      io.openshift.tags="ory,kratos,identity,ubi9,vetted"
...
COPY --from=builder /out/kratos /usr/bin/kratos
...
RUN set -euo pipefail; \
    ...
    printf 'ory:x:1001:0:Trusted Ory Kratos service account:/home/ory:/sbin/nologin\n' >> /etc/passwd; \
    mkdir -p /etc/pki/tls/certs /etc/ssl /home/ory /var/lib/sqlite; \
    ...
    chown -R 1001:0 /home/ory /var/lib/sqlite; \
    chmod -R g=u /home/ory /var/lib/sqlite
...
# The SQLite DSN default writes here. A bind mount not pre-owned by 1001 is
# the most common first-run failure, so the ownership above matters.
