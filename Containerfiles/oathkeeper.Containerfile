...
RUN set -euo pipefail; \
    GOARCH="${TARGETARCH}" go build -tags=json1 \
      -ldflags="-s -w \
        -X 'github.com/ory/oathkeeper/x.Version=${SOURCE_VERSION}' \
        -X 'github.com/ory/oathkeeper/x.Commit=${SOURCE_REF}' \
        -X 'github.com/ory/oathkeeper/x.Date=${SOURCE_DATE}'" \
      -o /out/oathkeeper . ; \
    /out/oathkeeper version
...
LABEL org.opencontainers.image.source="${SOURCE_URL}" \
      ...
      org.opencontainers.image.title="Trusted Ory Oathkeeper" \
      org.opencontainers.image.description="Ory Oathkeeper identity and access proxy, built from source on trusted UBI9 Micro" \
      ...
      io.openshift.tags="ory,oathkeeper,proxy,ubi9,vetted"
...
COPY --from=builder /out/oathkeeper /usr/bin/oathkeeper
...
RUN set -euo pipefail; \
    ...
    printf 'ory:x:1001:0:Trusted Ory Oathkeeper service account:/home/ory:/sbin/nologin\n' >> /etc/passwd; \
